"""MultiScale Hybrid v3 — Factual 1D backbone + Hourly encoder + OHLC regression.

Изменения по сравнению с v2:
  1. HourlyEncoder: часовые свечи → intraday token (мультитаймфрейм)
  2. Asymmetric Focal Loss: разный gamma для BUY/HOLD/SELL (чиним SELL recall)
  3. 8 токенов в cross-scale attention (было 7): +1 hourly token
  4. Гейтовое слияние: Gated Residual Network для trunk (по мотивам TFT)
  5. SELL-aware class weights: усиленный SELL weight
"""
import torch
...
def mixup_data(imgs_dict, num_dict, cls_y, ohlc_y, ctx, alpha=0.2,
    ...
    Теперь поддерживает hourly данные.
    """
    if alpha > 0:
...
# ══════════════════════════════════════════════════════════════════
#  Asymmetric Focal Loss (v3: per-class gamma)
# ══════════════════════════════════════════════════════════════════

class AsymmetricFocalLoss(nn.Module):
    """Focal Loss с разным gamma для каждого класса.

    Для SELL (class 2): gamma=1.0 → менее агрессивное подавление
    Для BUY  (class 0): gamma=2.0 → стандартный
    Для HOLD (class 1): gamma=3.0 → больше подавление easy samples

    Это заставляет модель больше фокусироваться на SELL-примерах.
    """
    def __init__(self, gamma_per_class=(2.0, 3.0, 1.0),
                 weight=None, label_smoothing=0.0):
        super().__init__()
        self.gamma = gamma_per_class  # (BUY, HOLD, SELL)
...
class GRN(nn.Module):
    ...
    Гейтовый механизм позволяет модели решать, сколько информации
    пропускать из каждого пути. Заменяет простой MLP в trunk.
    """
...
class MultiScaleHybridV3(nn.Module):
    ...
    Гибридная архитектура v3:
      ...
      - HourlyEncoder (часовые свечи 5 дней)                 → 128 × 1  ← NEW
      - Cross-scale Multi-Head Attention (8 токенов × 128)
      - GRN trunk (вместо простого MLP)                      ← NEW
    ...
    def __init__(self, ctx_dim: int = 0,
        ...
        if use_hourly:
            self.hourly_enc = HourlyEncoder(
                ...
                d_out=128, n_heads=2, dropout=0.2)

        # ── Cross-scale Attention ─────────────────────────────
        # 4 (backbone) + 2 (tcn) + 1 (lstm) + 1 (hourly) = 8 токенов × 128
    ...
    def forward(self, imgs_by_scale: dict,
        ...
        """
        imgs_by_scale: {W: (B, N_RENDER_CHANNELS, W)}
        num_by_scale:  {W: (B, W, n_indicator_cols)}
        ctx:           (B, ctx_dim)
        hourly:        (B, N_days, N_HOURLY_CHANNELS, N_HOURS_PER_DAY)  ← NEW
        ...
        if self.use_hourly and hourly is not None:
            hourly_tok = self.hourly_enc(hourly)  # (B, 128)
            tokens.append(hourly_tok)
        ...
        # ── Cross-scale Attention ─────────────────────────────
        seq = torch.stack(tokens, dim=1)          # (B, 8, 128)
        attn_out, _ = self.attn(seq, seq, seq)
        seq   = self.attn_norm(seq + attn_out)    # residual
        fused = seq.flatten(1)                     # (B, 1024)

        if ctx is not None:
            fused = torch.cat([fused, ctx], dim=1)

        # ── GRN Trunk → две головы ───────────────────────────
        h = self.trunk(fused)
        cls_logits = self.cls_head(h)
        ohlc_flat  = self.reg_head(h)
        ohlc_pred  = ohlc_flat.view(-1, self.future_bars, 4)

        return cls_logits, ohlc_pred


# ══════════════════════════════════════════════════════════════════
#  Multi-task Loss v3 (Asymmetric Focal)
# ══════════════════════════════════════════════════════════════════

class MultiTaskLossV3(nn.Module):
    """
    L = α × AsymmetricFocalLoss(cls) + (1-α) × HuberLoss(ohlc_reg)
    α — learnable (uncertainty weighting).

    SELL fix: gamma_per_class=(2.0, 3.0, 1.0)
    """
    def __init__(self, cls_weight=None,
        ...
        self.huber = nn.HuberLoss(delta=0.02)
...
def get_device():
    if torch.cuda.is_available():
        ...
        print(f"  Устройство: {p.name} ({p.total_mem // 1024**2} MB VRAM)"
              if hasattr(p, 'total_mem') else
              f"  Устройство: {p.name} ({p.total_memory // 1024**2} MB VRAM)")
    else:
        d = torch.device('cpu')
...
def _collate_v3(batch):
    """Collate для v3 формата: +hourly данные."""
    items = list(zip(*batch))
    # items: (imgs_list, num_list, cls_list, ohlc_list, ctx_list, hourly_list)
...
# ══════════════════════════════════════════════════════════════════
#  train_multiscale_v3 — стандартный режим
# ══════════════════════════════════════════════════════════════════
...
def train_multiscale_v3(tr_ds, y_tr, val_ds, y_val, ctx_dim,
    ...
    # Стандартная формула: sum / (3 * count)
    # Дополнительно умножаем SELL weight на 2.0
    base_weights = counts.sum() / (3 * counts + 1)
    base_weights[2] *= 2.0  # SELL boost
    ...
    criterion = MultiTaskLossV3(
        cls_weight=weights,
        gamma_per_class=(2.0, 3.0, 1.0),  # BUY, HOLD, SELL
